/* La Ribera Audiovisual — CMS público v2 */
(function(){
  'use strict';
  const CONTENT_ROW_ID = 'main';
  const DEFAULT_CONTENT = window.RIBERA_CONTENT || { site:{}, seo:{}, services:[], heroCarousel:[], portfolioWorks:[] };
  ...
  function normalizeContent(raw){
    ...
    if(!Array.isArray(c.services) || !c.services.length){
      c.services = [
        {id:'srv-01',title:'PRODUCCIÓN PUBLICITARIA',description:'Avisos, spots y piezas de campaña con dirección creativa integral. De la idea a la pantalla.',tag:'Campaña · Spot · Aviso',icon:'play',visible:true},
        {id:'srv-02',title:'VIDEOCLIP',description:'Producción de videoclips para artistas y bandas. Concepto visual, dirección, locación y post-producción.',tag:'Artistas · Bandas · Música',icon:'video',visible:true},
        {id:'srv-03',title:'IDENTIDAD EN VIDEO',description:'Construcción narrativa de la identidad de marca a través del lenguaje audiovisual.',tag:'Reel · Institucional · Marca',icon:'film',visible:true},
        {id:'srv-04',title:'CONTENIDO DIGITAL',description:'Piezas para redes, plataformas y canales propios. Pensadas para durar y para circular.',tag:'Redes · Plataformas · Digital',icon:'grid',visible:true},
        {id:'srv-05',title:'MARCA PERSONAL',description:'Visibilizamos quién sos. Para profesionales, artistas, creadores y fundadores.',tag:'Personal · Artistas · Profesionales',icon:'person',visible:true}
      ];
    }
  ...
  function mergeContent(remote){
    ...
    return Object.assign(base, c, {
      ...
      services:Array.isArray(remote.services) ? remote.services : base.services,
  ...
  function renderServices(c){
    ...
    grid.innerHTML=services.map((s,i)=>{const hasCover=!!(s.mediaUrl||s.url);return `<div class="service-card reveal visible rd${(i%4)+1} ${hasCover?'has-cover':''}">
      ${hasCover?`<div class="service-cover">${mediaMarkup({type:s.type||'image',url:s.mediaUrl||s.url,title:s.title,alt:s.alt||s.title},{opacity:'.84',autoplay:true})}</div>`:''}
  ...
  function openWork(id){
    ...
    const s=state.content.site||{}; const wa=String(s.whatsappWa||'5491132265225').replace(/\D/g,''); const text=encodeURIComponent(`Hola Ribera, vi el proyecto ${w.title||''} y me interesa hacer algo parecido.`); const cta=document.getElementById('modal-cta'); cta.href=`https://wa.me/${wa}?text=${text}`; cta.onclick=()=>trackEvent('whatsapp_click',{source:'portfolio_modal',work_id:w.id,title:w.title});
    renderModalMedia(); document.getElementById('work-modal').classList.add('open'); document.body.style.overflow='hidden'; trackEvent('portfolio_open',{work_id:w.id,title:w.title,category:w.category});
  }
  ...
  async function init(){
    ...
    applySEO(state.content); applySiteText(state.content); applyContact(state.content); renderServices(state.content); renderPhotography(state.content); renderHero(state.content); renderHomePortfolio(state.content); renderPortfolioPage(state.content); startCarousel();
